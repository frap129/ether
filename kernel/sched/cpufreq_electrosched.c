...
#define MAX_FREQ_SUSPEND	960000 /* 960MHz default */
 ...
 * @requested_freq: last frequency requested by the electrosched governor
 *
 * struct gov_data is the per-policy cpufreq_electrosched-specific data structure. A
 * per-policy instance of it is created when the cpufreq_electrosched governor receives
...
struct gov_data {
	...
	int max;
	unsigned int max_freq_suspend;
};

 /* worker thread for dvfs transition that may block/sleep */
static struct task_struct *freq_change_task;
static LIST_HEAD(electrosched_gov_list);
static struct mutex gov_list_lock;

/* irq_work: callback used to wake up worker thread */
static struct irq_work irq_work;

static void cpufreq_electrosched_try_driver_target(struct cpufreq_policy *policy, unsigned int freq)
{
	struct gov_data *gd;

	/* avoid race with cpufreq_electrosched_stop */
	...
	else if (freq > gd->max_freq_suspend)
		__cpufreq_driver_target(policy, gd->max_freq_suspend, CPUFREQ_RELATION_C);
	else
 ...
 * new policy pointer
 */
static int cpufreq_electrosched_thread(void *data)
{
	...
	do {
		...
		list_for_each_entry(gd, &electrosched_gov_list, gov_list) {
		...
		if (!policy) {
			...
			cpufreq_electrosched_try_driver_target(policy, freq);
		}
...
static void cpufreq_electrosched_irq_work(struct irq_work *irq_work)
{
	wake_up_process(freq_change_task);
}

/**
 * cpufreq_electrosched_set_capacity - interface to scheduler for changing capacity values
 * @cpu: cpu whose capacity utilization has recently changed
 * @capacity: the new capacity requested by cpu
 *
 * cpufreq_electrosched_sched_capacity is an interface exposed to the scheduler so
 ...
 *
 * cpufreq_electrosched_set_capacity raises an IPI. The irq_work handler for that IPI
 * wakes up the thread that does the actual work, cpufreq_electrosched_thread.
 *
...
void cpufreq_electrosched_set_cap(int cpu, unsigned long capacity)
{
...
/**
 * cpufreq_electrosched_reset_capacity - interface to scheduler for resetting capacity
 *                                requests
 * @cpu: cpu whose capacity request has to be reset
 *
 * This _wont trigger_ any capacity update.
 */
void cpufreq_electrosched_reset_cap(int cpu)
{
	per_cpu(pcpu_capacity, cpu) = 0;
}

static inline void set_electrosched_energy_freq(void)
{
...
static inline void clear_electrosched_energy_freq(void)
{
	// TJK: must always bump the key so we can handle
	// 1 cluster going online/offline
...
static int cpufreq_electrosched_policy_start(struct cpufreq_policy *policy)
{
	int cpu;
...
static int cpufreq_electrosched_policy_stop(struct cpufreq_policy *policy)
{
	int cpu;
	if (!policy) {
		return 0;
...
static struct attribute_group electrosched_attr_group_gov_pol;
static struct attribute_group *get_sysfs_attr(void)
{
	return &electrosched_attr_group_gov_pol;
}

static int cpufreq_electrosched_policy_init(struct cpufreq_policy *policy)
{
	int rc;
	struct gov_data *gd;
	...
	gd->max_freq_suspend = MAX_FREQ_SUSPEND;
	...
	mutex_lock(&gov_list_lock);
	list_add_tail(&gd->gov_list, &electrosched_gov_list);
	mutex_unlock(&gov_list_lock);

	set_electrosched_energy_freq();
...
static int cpufreq_electrosched_policy_exit(struct cpufreq_policy *policy)
{
	...
	clear_electrosched_energy_freq();
...
static int cpufreq_electrosched_setup(struct cpufreq_policy *policy, unsigned int event)
{
	struct gov_data *gd;
	switch (event) {
		case CPUFREQ_GOV_START:
			return cpufreq_electrosched_policy_start(policy);
		case CPUFREQ_GOV_STOP:
			return cpufreq_electrosched_policy_stop(policy);
		case CPUFREQ_GOV_POLICY_INIT:
			return cpufreq_electrosched_policy_init(policy);
		case CPUFREQ_GOV_POLICY_EXIT:
			return cpufreq_electrosched_policy_exit(policy);
...
static ssize_t show_max_freq_suspend(struct gov_data *gd, char *buf)
{
	return sprintf(buf, "%u\n", gd->max_freq_suspend);
}

static ssize_t store_max_freq_suspend(struct gov_data *gd,
		const char *buf, size_t count)
{
	int ret;
	long unsigned int val;
...
tunable_handlers(max_freq_suspend);

/* Per policy governor instance */
static struct attribute *electrosched_attributes_gov_pol[] = {
	&up_throttle_nsec_gov_pol.attr,
	&down_throttle_nsec_gov_pol.attr,
	&max_freq_suspend_gov_pol.attr,
	NULL,
};

static struct attribute_group electrosched_attr_group_gov_pol = {
	.attrs = electrosched_attributes_gov_pol,
	.name = "electrosched",
};

#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_ELECTROSCHED
static
#endif
struct cpufreq_governor cpufreq_gov_electrosched = {
	.name			= "electrosched",
	.governor		= cpufreq_electrosched_setup,
	.owner			= THIS_MODULE,
};

static int __init cpufreq_electrosched_init(void)
{
	int cpu;
	...
	freq_change_task = kthread_run(cpufreq_electrosched_thread, NULL, "kcpufreq_electrosched_task");
	if (IS_ERR_OR_NULL(freq_change_task)) {
			pr_err("%s: failed to create kcpufreq_electrosched_task thread\n", __func__);
		return -ENOMEM;
	}

	init_irq_work(&irq_work, cpufreq_electrosched_irq_work);
	return cpufreq_register_governor(&cpufreq_gov_electrosched);
}

static void __exit cpufreq_electrosched_exit(void)
{
	kthread_stop(freq_change_task);
	cpufreq_unregister_governor(&cpufreq_gov_electrosched);
}

/* Try to make this the default governor */
fs_initcall(cpufreq_electrosched_init);
